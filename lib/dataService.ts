...
class DataService {
  ...
  private coletasFile = path.join(this.dataDir, "coletas.json")
  private emailsFile = path.join(this.dataDir, "email-subscriptions.json")
  private templateFile = path.join(this.dataDir, "email-template.json")
  ...
  async getColetaByDia(diaSemana: string): Promise<Coleta | null> {
    const coletas = await this.getColetas()
    return coletas.find((c) => c.dia_semana.toLowerCase() === diaSemana.toLowerCase()) || null
  }

  async updateColeta(diaSemana: string, tipoColeta: string, observacao?: string): Promise<Coleta | null> {
    const data = await this.readJSONFile(this.coletasFile)
    const coletaIndex = data.coletas.findIndex((c: Coleta) => c.dia_semana.toLowerCase() === diaSemana.toLowerCase())
  ...
  async addEmailSubscription(email: string) {
    ...
    const existingIndex = data.subscriptions.findIndex((s: EmailSubscription) => s.email === email)
    ...
    data.metadata.active_subscriptions = data.subscriptions.filter((s: EmailSubscription) => s.is_active).length
    data.metadata.last_updated = new Date().toISOString()
  ...
  async getActiveEmails(): Promise<string[]> {
    const data = await this.getEmailSubscriptions()
    return data.subscriptions.filter((s: EmailSubscription) => s.is_active).map((s: EmailSubscription) => s.email)
  }

  async unsubscribeByToken(token: string) {
    const data = await this.getEmailSubscriptions()
    const subscriptionIndex = data.subscriptions.findIndex((s: EmailSubscription) => s.unsubscribe_token === token)
  ...
  async updateUnsubscribeToken(email: string, token: string) {
    const data = await this.getEmailSubscriptions()
    const subscriptionIndex = data.subscriptions.findIndex((s: EmailSubscription) => s.email === email && s.is_active)
  ...
  async getEmailStats() {
    const data = await this.getEmailSubscriptions()
    return {
      activeSubscriptions: data.subscriptions.filter((s: EmailSubscription) => s.is_active).length,
      totalSubscriptions: data.subscriptions.length,
      inactiveSubscriptions: data.subscriptions.filter((s: EmailSubscription) => !s.is_active).length,
    }
  }
